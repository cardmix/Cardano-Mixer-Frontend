  ...
  divClass "columns-4 w-row" $ mdo
    ...
    eConnect <- divClass colCls8 $ do
      eeConnect <- dyn $ dFormState <&> \case
        Deposit -> depositForm dWalletName dWalletConnected
        Withdraw -> withdrawForm dWalletName >> return never
      switchHold never eeConnect
    performEvent_ (tagPromptlyDyn (flip JS.enable elemId <$> dWalletName) $ leftmost [eCardmixLoaded, eConnect])
